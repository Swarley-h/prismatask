...
test.group('Tasks - Get By User Id', (group) => {
  ...
  test('should get task successfully', async ({ assert }) => {
    const task = await taskServices.getTaskByUserId(1)

    assert.isTrue(task[0].id === 1 && task[0].description === 'Testando')
  })

  test('should fail when there is not task created', async ({ assert }) => {
    await assert.rejects(() => taskServices.getTaskByUserId(2), 'Usuário não encontrado')
  })
})

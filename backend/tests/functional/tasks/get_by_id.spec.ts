...
test.group('Task - Get By Id', async (group) => {
  ...
  test('should get task successfully', async ({ client }) => {
    const response = await client.get('/tasks/1').header('Authorization', `Bearer ${tokenString}`)

    response.assertStatus(200)
  })

  test('should fail when task does not exist', async ({ client, assert }) => {
    const response = await client.put('/tasks/2').header('Authorization', `Bearer ${tokenString}`)
